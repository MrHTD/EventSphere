...
export const BoothAllocations = () => {
  ...
  const user_id = JSON.parse(localStorage.getItem('publicuser'));
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/getboothAllocations')
      .then(response => {
        setBoothsAllocation(response.data.filter(data => data.userId === user_id._id));
                        ...
                        <table className="table text-nowrap mb-0 align-middle text-center">
                          <thead className="text-dark fs-4">
                            <tr>
                              ...
                              <th className="border-bottom-0">
                                <h6 className="fw-semibold mb-0">You Reserved</h6>
                              </th>
                              <th className="border-bottom-0">
                              ...
                              paginatedData.map((boothAllocation, index) => {
                                ...
                                return (
                                  <tr key={index}>
                                    ...
                                    <td className="border-bottom-0">
                                      <Link to={`/`} type="button" className="btn btn-dark m-1">Unreserved</Link>
                                    </td>
                                  </tr>
                                );
                              })
