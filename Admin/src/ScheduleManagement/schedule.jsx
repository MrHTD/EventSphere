                ...
                <div className="col-lg-12 d-flex align-items-stretch">
                  <div className="card w-100">
                    <div className="card-body p-4">
                      <Row>
                        <Col>
                          <h5 className="card-title fw-semibold mb-4">Schedule Management</h5>
                        </Col>
                        <Col className='text-end'>
                          <Link to="/addschedule" type="button" className="w-auto btn btn-primary">Add Schedule</Link>
                        </Col>
                      </Row>
                          ...
                          <tbody>
                            {
                              paginatedData.map((schedule, index) => (
                                <tr key={index}>
                                  <td className="border-bottom-0"><h6 className="fw-semibold mb-0">{schedule.title}</h6></td>
                                  <td className="border-bottom-0"><h6 className="fw-semibold mb-0">{schedule.description}</h6></td>
                                  <td className="border-bottom-0">
                                    <p className="mb-0 fw-normal">{new Date(schedule.date).toLocaleDateString()}</p>
                                  </td>
                                  <td className="border-bottom-0">
                                    <h6 className="fw-semibold mb-1"></h6>
                                    <span className="fw-normal">{schedule.location}</span>
                                  </td>
                                  <td className="border-bottom-0">
                                    <Link to={`/editexpoevent/${schedule._id}`} type="button" className="btn btn-success m-1">Edit</Link>
                                    <Link to={`/deleteexpoevent/${schedule._id}`} type="button" className="btn btn-danger m-1">Delete</Link>
                                  </td>
                                </tr>
                              )
                              )
